...
impl ParallelExecutor {
    ...
    pub fn parallel_hash_join(
        ...
        #[cfg(feature = "parallel")]
        let right_hash: std::collections::HashMap<u64, Vec<Row>> = right.rows
            .into_par_iter()
            .fold(
        ...
        #[cfg(feature = "parallel")]
        let joined_rows: QueryResult<Vec<Row>> = left.rows
            .into_par_iter()
            .map(|left_row| {
                // Check cancellation
                if let Some(ctx) = checkpoint_ctx {
    ...
    fn sequential_hash_join(
        ...
        let mut right_hash: std::collections::HashMap<u64, Vec<Row>> = std::collections::HashMap::new();
        for row in right.rows {
            let key = join_key_fn(&row);
...
mod tests {
    ...
    fn test_parallel_scan() {
        ...
        let result = executor.parallel_scan(
            100,
            |start, end| {
                Ok((start..end).map(|i| Row::new(vec![crate::ast::Value::Int(i as i64)])).collect())
            },
            None,
        ).unwrap();
    ...
    fn test_parallel_filter() {
        ...
        let filtered = executor.parallel_filter(
            rows,
    ...
    fn test_parallel_aggregate() {
        ...
        let aggregated = executor.parallel_aggregate(
            rows,
            |row| row.values[0].as_string().unwrap_or_default(),
            |group_rows| {
