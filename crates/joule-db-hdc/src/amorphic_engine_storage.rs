//! Amorphic Storage Engine
//!
//! Persistent B-tree storage integration for the Amorphic Engine.
//!
//! This module provides durable storage for all AmorphicEngine subsystems:
//! - SDM hard locations
...
//! - `__amorphic__::sdm::` - Sparse Distributed Memory
//! - `__amorphic__::holo::` - Holographic patterns
//! - `__amorphic__::hd::` - Hyperdimensional vectors
//! - `__amorphic__::learned::` - Learned index models
//! - `__amorphic__::snn::` - Spiking Neural Network weights
//! - `__amorphic__::meta::` - Metadata and configuration
//!
//! ## Example
//!
//! ```rust,ignore
//! use joule_db_hdc::amorphic_engine_storage::{AmorphicStorageEngine, AmorphicStorageConfig};
...
//! let storage = AmorphicStorageEngine::new(engine, AmorphicStorageConfig::default());
//!
//! // Store data with AmorphicEngine indexing
//! storage.put("user:1", b"Alice").unwrap();
//!
//! // Hybrid query combining B-tree + AmorphicEngine
...
use crate::amorphic_engine::{
    AmorphicEngineConfig, AmorphicEngineError, AmorphicEngineResult, AmorphicEngineStats,
};
...
// ============================================================================
// Key Prefixes (reserved for full persistence implementation)
// ============================================================================

/// Key prefix for SDM data
const PREFIX_SDM: &[u8] = b"__amorphic__::sdm::";
/// Key prefix for holographic patterns
const PREFIX_HOLO: &[u8] = b"__amorphic__::holo::";
/// Key prefix for hyperdimensional vectors
const PREFIX_HD: &[u8] = b"__amorphic__::hd::";
/// Key prefix for learned index models
const PREFIX_LEARNED: &[u8] = b"__amorphic__::learned::";
/// Key prefix for SNN weights
const PREFIX_SNN: &[u8] = b"__amorphic__::snn::";
/// Key prefix for metadata
const PREFIX_META: &[u8] = b"__amorphic__::meta::";
/// Key prefix for user data
const PREFIX_DATA: &[u8] = b"__amorphic__::data::";
...
/// Configuration for Amorphic Storage Engine
#[derive(Clone, Debug)]
pub struct AmorphicStorageConfig {
    /// AmorphicEngine configuration (dimensions, etc.)
    pub engine: AmorphicEngineConfig,
    ...
    /// Batch size for persisting AmorphicEngine state
    ...
    /// Weight for AmorphicEngine results in hybrid queries (0.0-1.0)
    pub amorphic_weight: f64,
}

impl Default for AmorphicStorageConfig {
    fn default() -> Self {
        Self {
            engine: AmorphicEngineConfig::default(),
            auto_tune: true,
...
impl AmorphicStorageConfig {
    /// Create minimal configuration for testing
    pub fn minimal() -> Self {
        Self {
            engine: AmorphicEngineConfig::minimal(),
            auto_tune: false,
    ...
    pub fn production() -> Self {
        Self {
            engine: AmorphicEngineConfig::production(),
            auto_tune: true,
...
// ============================================================================
// Serialization Helpers (reserved for full persistence)
// ============================================================================
...
fn deserialize_sdm_location(data: &[u8]) -> AmorphicEngineResult<(Vec<u8>, Vec<i32>, u32)> {
    if data.len() < 12 {
        return Err(AmorphicEngineError::Other(
            "SDM location data too short".into(),
        ));
    }
    ...
    if magic != MAGIC_SDM_LOCATION {
        return Err(AmorphicEngineError::Sdm(
            "Invalid SDM location magic".into(),
        ));
    }
    cursor += 4;
    ...
    if cursor + addr_len > data.len() {
        return Err(AmorphicEngineError::Sdm("Address data truncated".into()));
    }
    ...
    if cursor + 4 > data.len() {
        return Err(AmorphicEngineError::Sdm("Missing counter length".into()));
    }
    ...
    for _ in 0..counter_len {
        if cursor + 4 > data.len() {
            return Err(AmorphicEngineError::Sdm("Counter data truncated".into()));
        }
    ...
    // Write count
    if cursor + 4 > data.len() {
        return Err(AmorphicEngineError::Sdm("Missing write count".into()));
    }
...
fn deserialize_holo_pattern(data: &[u8]) -> AmorphicEngineResult<(String, Vec<f32>)> {
    if data.len() < 8 {
        return Err(AmorphicEngineError::Holographic(
            "Pattern data too short".into(),
        ));
    }
...
fn deserialize_hd_vector(data: &[u8]) -> AmorphicEngineResult<(usize, Vec<f32>)> {
    if data.len() < 16 {
        return Err(AmorphicEngineError::Hyperdimensional(
            "Vector data too short".into(),
        ));
    }
...
fn deserialize_learned_model(
    data: &[u8],
) -> AmorphicEngineResult<(u8, Vec<f64>, f64, f64, usize, f64, f64)> {
    if data.len() < 45 {
        return Err(AmorphicEngineError::Learned("Model data too short".into()));
    }
    ...
    if magic != MAGIC_LEARNED_MODEL {
        return Err(AmorphicEngineError::Learned("Invalid model magic".into()));
    }
    cursor += 4;
    ...
    for _ in 0..coef_len {
        if cursor + 8 > data.len() {
            return Err(AmorphicEngineError::Learned(
                "Coefficient data truncated".into(),
            ));
        }
...
fn deserialize_synapse(data: &[u8]) -> AmorphicEngineResult<Synapse> {
    if data.len() < 20 {
        return Err(AmorphicEngineError::Snn("Synapse data too short".into()));
    }

    // Verify magic
    let magic = u32::from_le_bytes([data[0], data[1], data[2], data[3]]);
    if magic != MAGIC_SNN_SYNAPSE {
        return Err(AmorphicEngineError::Snn("Invalid synapse magic".into()));
    }
...
fn deserialize_neuron(data: &[u8]) -> AmorphicEngineResult<Neuron> {
    // Magic (4) + type (1) + threshold (4) + potential (4) + last_spike_time (4) = 17 bytes
    if data.len() < 17 {
        return Err(AmorphicEngineError::Snn("Neuron data too short".into()));
    }

    // Verify magic
    let magic = u32::from_le_bytes([data[0], data[1], data[2], data[3]]);
    if magic != MAGIC_SNN_NEURON {
        return Err(AmorphicEngineError::Snn("Invalid neuron magic".into()));
    }
...
/// Statistics for AmorphicStorage engine
#[derive(Debug, Clone)]
pub struct AmorphicStorageStats {
    /// Total keys stored
    pub total_keys: u64,
    /// B-tree operations
    pub btree_reads: u64,
    ...
    /// AmorphicEngine subsystem stats
    pub amorphic_stats: AmorphicEngineStats,
...
impl QueryPatternTracker {
    ...
    fn calculate_optimal_weights(&self) -> (f64, f64) {
        ...
        let _amorphic_ratio = (sim + content) / total;

        // Normalize to ensure they sum to 1.0
        let btree_weight = 0.3 + (btree_ratio * 0.5); // Range: 0.3-0.8
        let amorphic_weight = 1.0 - btree_weight;

        (btree_weight, amorphic_weight)
    }
}
...
/// Amorphic Storage Engine combining B-tree storage with AmorphicEngine indexing
pub struct AmorphicStorageEngine {
    /// Configuration
    config: AmorphicStorageConfig,

    /// Persistent B-Tree engine
    engine: Arc<Engine>,

    /// AmorphicEngine subsystems
...
impl AmorphicStorageEngine {
    /// Create a new AmorphicStorage engine
    pub fn new(engine: Arc<Engine>, config: AmorphicStorageConfig) -> Self {
        let engine_config = &config.engine;

        let sdm = SparseDistributedMemory::new(
            engine_config.sdm_locations,
            engine_config.sdm_dimension,
            engine_config.sdm_data_size,
        );

        let holographic = HolographicStorage::new(engine_config.holographic_dimension);
        let hyperdimensional =
            HyperdimensionalStorage::new(engine_config.hyperdimensional_dimension);
        let snn = SpikeNeuralNetwork::new(engine_config.snn_neurons, engine_config.snn_time_step);
    ...
    pub fn minimal() -> Self {
        let backend = MemoryBackend::new();
        let engine = Arc::new(Engine::new(backend).unwrap());
        Self::new(engine, AmorphicStorageConfig::minimal())
    }
    ...
    /// Put a key-value pair with AmorphicEngine indexing
    pub fn put(&self, key: &str, value: &[u8]) -> AmorphicEngineResult<()> {
        // Update stats
        ...
        self.engine
            ...
            .map_err(|e| AmorphicEngineError::Other(e.to_string()))?;
    ...
    /// Execute a hybrid query combining B-tree and AmorphicEngine results
    pub fn hybrid_query(
        &self,
        query_data: &[u8],
        top_k: usize,
    ) -> AmorphicEngineResult<Vec<HybridQueryResult>> {
        if !self.config.enable_hybrid_queries {
            return Err(AmorphicEngineError::Other("Hybrid queries disabled".into()));
        }
        ...
        let (btree_weight, amorphic_weight) = if self.config.auto_tune {
            self.pattern_tracker.calculate_optimal_weights()
        } else {
            (self.config.btree_weight, self.config.amorphic_weight)
        };

        // 1. SDM content lookup
        self.add_sdm_results(&mut results, query_data, amorphic_weight)?;

        // 2. Holographic associative search
        self.add_holographic_results(&mut results, query_data, amorphic_weight)?;

        // 3. Hyperdimensional similarity search
        self.add_hyperdimensional_results(&mut results, query_data, top_k, amorphic_weight)?;
    ...
    // ========================================================================
    // AmorphicEngine Indexing
    // ========================================================================

    fn index_in_sdm(&self, key: &str, value: &[u8]) -> AmorphicEngineResult<()> {
        if let Ok(sdm) = self.sdm.read() {
            let address = SDMAddress::from_data(key.as_bytes(), self.config.engine.sdm_dimension);
            let _ = sdm.write_bytes(&address, value);
        }
        Ok(())
    }

    fn index_in_holographic(&self, key: &str, value: &[u8]) -> AmorphicEngineResult<()> {
        if let Ok(holo) = self.holographic.read() {
            // Convert value to complex pattern
            let dim = self.config.engine.holographic_dimension;
    ...
    fn add_hyperdimensional_results(
        ...
        let query_hv = HyperVector::random(self.config.engine.hyperdimensional_dimension, seed);
    ...
    fn persist_data(&self, key: &str, value: &[u8]) -> AmorphicEngineResult<()> {
    ...
    pub fn persist_sdm(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
    ...
    pub fn load_sdm(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
    ...
    pub fn persist_holographic(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
        ...
        let meta_value = self.config.engine.holographic_dimension.to_string();
    ...
    pub fn load_holographic(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
    ...
    pub fn persist_hyperdimensional(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
        ...
        let meta_value = self.config.engine.hyperdimensional_dimension.to_string();
    ...
    pub fn load_hyperdimensional(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
    ...
    pub fn persist_learned_index(&self) -> AmorphicEngineResult<usize> {
    ...
    pub fn load_learned_index(&self) -> AmorphicEngineResult<usize> {
        let persist_key = [PREFIX_LEARNED, b"model"].concat();
    ...
    pub fn persist_snn(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
    ...
    pub fn load_snn(&self) -> AmorphicEngineResult<usize> {
        let mut count = 0;
    ...
    /// Persist all AmorphicEngine state
    pub fn persist_all(&self) -> AmorphicEngineResult<usize> {
        let mut total = 0;
    ...
    /// Load all AmorphicEngine state from storage
    pub fn load_all(&self) -> AmorphicEngineResult<usize> {
        let mut total = 0;
        total += self.load_sdm()?;
    ...
    fn maybe_train_learned_index(&self) -> AmorphicEngineResult<()> {
        let len = self.len();
    ...
    pub fn train_learned_index(&self) -> AmorphicEngineResult<(f64, f64)> {
        ...
        if keys.len() < 2 {
            return Err(AmorphicEngineError::Learned(
                "Need at least 2 keys to train".into(),
            ));
        }
    ...
    pub fn stats(&self) -> AmorphicStorageStats {
        ...
        let amorphic_stats = AmorphicEngineStats {
    ...
    /// Get configuration
    pub fn config(&self) -> &AmorphicStorageConfig {
        &self.config
    }
}
...
mod tests {
    ...
    fn test_engine_creation() {
        let engine = AmorphicStorageEngine::minimal();
    ...
    fn test_learned_index_training() {
        ...
        let engine = AmorphicStorageEngine::new(
            core_engine,
            AmorphicStorageConfig {
                learned_index_threshold: 5,
                ..AmorphicStorageConfig::minimal()
            },
        );
    ...
    fn test_auto_tuning_weights() {
        ...
        let (btree_weight, amorphic_weight) = engine.current_weights();
        ...
        assert!(
            btree_weight + amorphic_weight > 0.99,
    ...
    fn test_config_variants() {
        let minimal = AmorphicStorageConfig::minimal();
        let production = AmorphicStorageConfig::production();
        let default_config = AmorphicStorageConfig::default();
