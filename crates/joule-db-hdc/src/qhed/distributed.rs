//! Distributed QHED Components
//!
//! Multi-node coordination for distributed QHED deployment.
...
impl VectorClock {
    ...
    pub fn to_json(&self) -> String {
        if let Ok(clock) = self.clock.read() {
            let pairs: Vec<String> = clock.iter().map(|(k, v)| format!("\"{}\":{}", k, v)).collect();
...
/// Simulated distributed QHED node
///
/// Represents a single node in a distributed QHED cluster.
...
impl DistributedCluster {
    ...
    pub fn write_to_node(&self, node_id: &str, key: &str, value: Vec<u8>) -> QHEDResult<()> {
        if let Ok(nodes) = self.nodes.read() {
            let node = nodes
                .get(node_id)
                .ok_or_else(|| QHEDError::NodeNotFound(node_id.to_string()))?;
            node.write(key, value);
            return Ok(());
        }
        Err(QHEDError::LockError("failed to acquire nodes lock".to_string()))
    }

    /// Read from a specific node
    pub fn read_from_node(&self, node_id: &str, key: &str) -> QHEDResult<Option<Vec<u8>>> {
        if let Ok(nodes) = self.nodes.read() {
            let node = nodes
                .get(node_id)
                .ok_or_else(|| QHEDError::NodeNotFound(node_id.to_string()))?;
...
mod tests {
    ...
    fn test_distributed_cluster_multiple_syncs() {
        ...
        // Write to node1
        cluster.write_to_node("node1", "key1", b"v1".to_vec()).unwrap();
        cluster.sync_all();

        // Write to node2
        cluster.write_to_node("node2", "key2", b"v2".to_vec()).unwrap();
        cluster.sync_all();

        // Write to node3
        cluster.write_to_node("node3", "key3", b"v3".to_vec()).unwrap();
        cluster.sync_all();
