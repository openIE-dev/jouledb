...
use super::{QHEDError, QHEDResult};
...
use crate::predictive::{NGramPredictor, QueryPredictor, Prediction};
use crate::sdm::{SDMAddress, SparseDistributedMemory};
use crate::spiking::{SpikeNeuralNetwork, SpikeEvent, TemporalEncoder, TemporalDecoder, SNNStats};
...
fn rand_simple() -> f64 {
    ...
    let x = n.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
    (x as f64) / (u64::MAX as f64)
}

/// QHED Configuration
#[derive(Clone, Debug)]
pub struct QHEDConfig {
...
/// Statistics about QHED instance
#[derive(Clone, Debug)]
pub struct QHEDStats {
    /// Number of reads
    pub reads: u64,
    /// Number of writes
    pub writes: u64,
...
/// Quantum-Holographic Emergent Database
///
...
pub struct QHED {
    ...
    // Metrics
    metrics: Arc<RwLock<QHEDMetrics>>,
...
impl QHED {
    /// Create new QHED instance with default configuration
    pub fn new() -> Self {
        Self::with_config(QHEDConfig::default(), "node_default")
    }

    /// Create QHED with custom configuration
    pub fn with_config(config: QHEDConfig, node_id: &str) -> Self {
        ...
        Self {
            ...
            metrics: Arc::new(RwLock::new(QHEDMetrics::default())),
    ...
    /// Write data with full QHED indexing
    pub fn write(&self, key: &str, value: &[u8]) -> QHEDResult<()> {
    ...
    pub fn store_content(&self, data: &[u8]) -> QHEDResult<String> {
    ...
    pub fn associative_search(&self, query: &[f32], top_k: usize) -> QHEDResult<Vec<SimilarityResult>> {
        if let Ok(mut m) = self.metrics.write() {
    ...
    pub fn holographic_recall(&self, key: &str, partial_query: &[f32]) -> QHEDResult<Vec<f32>> {
        if let Ok(mut m) = self.metrics.write() {
            m.holographic_lookups += 1;
        }
        ...
        Err(QHEDError::LockError("failed to acquire holographic lock".to_string()))
    }
    ...
    /// Add hypervector
    pub fn add_hypervector(&self, vector: &HyperVector) -> QHEDResult<usize> {
        if let Ok(hd) = self.hyperdimensional.read() {
            return hd
                .add_vector(vector.clone())
                .map_err(|e| QHEDError::Hyperdimensional(e.to_string()));
        }
        Err(QHEDError::LockError("failed to acquire hyperdimensional lock".to_string()))
    }
    ...
    pub fn train_learned_index(&self) -> QHEDResult<(f64, f64)> {
        ...
        let keys: Vec<String> = if let Ok(store) = self.data_store.read() {
            ...
            return Err(QHEDError::LockError("failed to acquire data store lock".to_string()));
        };

        if keys.len() < 2 {
            return Err(QHEDError::Learned("need at least 2 keys to train".to_string()));
        }
    ...
    /// Get configuration
    pub fn config(&self) -> &QHEDConfig {
        &self.config
    }
}

impl Default for QHED {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Clone for QHED {
    fn clone(&self) -> Self {
        // Create a new QHED with same config
...
mod tests {
    ...
    #[test]
    fn test_qhed_basic_operations() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Write
        qhed.write("key1", b"value1").unwrap();
        qhed.write("key2", b"value2").unwrap();

        // Read
        assert_eq!(qhed.read("key1"), Some(b"value1".to_vec()));
        assert_eq!(qhed.read("key2"), Some(b"value2".to_vec()));
        assert_eq!(qhed.read("key3"), None);

        // Contains
        assert!(qhed.contains("key1"));
        assert!(!qhed.contains("key3"));

        // Length
        assert_eq!(qhed.len(), 2);
        assert!(!qhed.is_empty());
    }
    ...
    fn test_qhed_delete() {
        ...
        assert!(qhed.delete("key1"));
        assert!(!qhed.contains("key1"));
        assert!(!qhed.delete("key1")); // Already deleted
    }
    ...
    #[test]
    fn test_qhed_content_store() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        let data = b"test content";
        let key = qhed.store_content(data).unwrap();
        assert!(key.starts_with("content_"));

        assert!(qhed.contains(&key));
        assert_eq!(qhed.read(&key), Some(data.to_vec()));
    }
    ...
    fn test_qhed_stats() {
        ...
        qhed.write("key1", b"value1").unwrap();
        qhed.read("key1");
        qhed.read("key1"); // Cache hit
        qhed.read("key2"); // Cache miss

        let stats = qhed.stats();
    ...
    #[test]
    fn test_qhed_config() {
        let minimal = QHEDConfig::minimal();
        assert_eq!(minimal.sdm_locations, 100);

        let production = QHEDConfig::production();
        assert_eq!(production.sdm_locations, 10000);

        let default_config = QHEDConfig::default();
        assert_eq!(default_config.sdm_locations, 1000);
    }

    #[test]
    fn test_qhed_node_id() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "my_node");
        assert_eq!(qhed.node_id(), "my_node");
    }

    #[test]
    fn test_qhed_optimizer() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        let plan = qhed.optimize_query(0.5, 2, true);
        assert!(plan.cost() > 0.0);

        let temp = qhed.optimizer_temperature();
        assert!(temp > 0.0);

        qhed.reset_optimizer();
    }

    #[test]
    fn test_qhed_predictions() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Generate some query patterns
        for _ in 0..5 {
            qhed.write("key1", b"v1").unwrap();
            let _ = qhed.read("key1");
            qhed.write("key2", b"v2").unwrap();
            let _ = qhed.read("key2");
        }

        let predictions = qhed.get_predictions(5);
    ...
    #[test]
    fn test_qhed_cache_hit_rate() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Initially 0
        assert_eq!(qhed.cache_hit_rate(), 0.0);

        qhed.write("key1", b"value1").unwrap();
        qhed.read("key1"); // Miss, then cached
        qhed.read("key1"); // Should be hit

        let hit_rate = qhed.cache_hit_rate();
        assert!(hit_rate >= 0.0 && hit_rate <= 1.0);
    }

    #[test]
    fn test_qhed_similarity_search() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        qhed.write("key1", b"hello world").unwrap();
        qhed.write("key2", b"hello there").unwrap();
        qhed.write("key3", b"goodbye world").unwrap();

        let results = qhed.similarity_search(b"hello", 3);
        // Results are NeighborResult with id, index, distance
        assert!(results.len() <= 3);
    }

    #[test]
    fn test_qhed_thread_safety() {
        use std::thread;

        let qhed = Arc::new(QHED::with_config(QHEDConfig::minimal(), "test_node"));
        let mut handles = vec![];

        // Multiple writers
        for i in 0..4 {
            let qhed_clone = Arc::clone(&qhed);
            handles.push(thread::spawn(move || {
                for j in 0..10 {
                    let key = format!("key_{}_{}", i, j);
                    let value = format!("value_{}_{}", i, j);
                    qhed_clone.write(&key, value.as_bytes()).unwrap();
                }
            }));
        }
        ...
        for _ in 0..4 {
            ...
            handles.push(thread::spawn(move || {
                for _ in 0..20 {
                    let _ = qhed_clone.read("key_0_0");
                    let _ = qhed_clone.len();
                }
            }));
        }
    ...
    #[test]
    fn test_qhed_ngram_predictions() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Generate N-gram pattern: a -> b -> c (repeated)
        for _ in 0..10 {
            qhed.write("pattern_a", b"a").unwrap();
            let _ = qhed.read("pattern_a");
            qhed.write("pattern_b", b"b").unwrap();
            let _ = qhed.read("pattern_b");
            qhed.write("pattern_c", b"c").unwrap();
            let _ = qhed.read("pattern_c");
        }

        // Get N-gram predictions
        let ngram_preds = qhed.get_ngram_predictions(5);
        // May or may not have predictions depending on pattern establishment
        assert!(ngram_preds.len() <= 5);

        // Get combined predictions
        let combined = qhed.get_combined_predictions(5);
        assert!(combined.len() <= 5);
    }

    #[test]
    fn test_qhed_learned_index() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Initially not trained
        assert!(!qhed.is_learned_index_trained());
        assert!(qhed.learned_index_stats().is_none());

        // Add enough keys to train
        for i in 0..100 {
            qhed.write(&format!("key_{:04}", i), format!("value_{}", i).as_bytes()).unwrap();
        }

        // Train the learned index
        let result = qhed.train_learned_index();
        assert!(result.is_ok());
        ...
        let stats = qhed.learned_index_stats();
    ...
    #[test]
    fn test_qhed_learned_index_not_enough_keys() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Only one key - not enough to train
        qhed.write("only_key", b"value").unwrap();
    ...
    #[test]
    fn test_qhed_snn_operations() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Get initial SNN stats
        let stats = qhed.snn_stats();
        assert!(stats.is_some());
        ...
        // Step the SNN
        let spikes = qhed.step_snn();
        // May or may not have spikes
        assert!(spikes.len() <= stats.num_neurons);

        // Simulate multiple steps
        let all_spikes = qhed.simulate_snn(10);
        assert_eq!(all_spikes.len(), 10);

        // Reset SNN
        qhed.reset_snn();
        let stats_after_reset = qhed.snn_stats().unwrap();
        assert_eq!(stats_after_reset.current_time, 0.0);
    }

    #[test]
    fn test_qhed_snn_access_recording() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Access patterns should be recorded in SNN
        for i in 0..10 {
            qhed.write(&format!("key_{}", i), b"value").unwrap();
            let _ = qhed.read(&format!("key_{}", i));
        }
    ...
    #[test]
    fn test_qhed_detect_anomalies() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Detect anomalies (should work even with no data)
        let anomalies = qhed.detect_access_anomalies();
        // May or may not have anomalies depending on SNN state
        assert!(anomalies.len() <= qhed.config().snn_neurons);
    }

    #[test]
    fn test_qhed_snn_encode_decode() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Encode temporal data
        let data = vec![0.1, 0.5, 0.9, 0.3, 0.7];
        let spikes = qhed.snn_encode(&data, 10.0);

        // Spikes should be generated
        assert!(!spikes.is_empty());

        // Decode back
        let decoded = qhed.snn_decode(&spikes, 10.0, 5);
        assert_eq!(decoded.len(), 5);
    }

    #[test]
    fn test_qhed_stats_include_new_metrics() {
        let qhed = QHED::with_config(QHEDConfig::minimal(), "test_node");

        // Write and read to generate metrics
        qhed.write("test", b"data").unwrap();
        let _ = qhed.read("test");
